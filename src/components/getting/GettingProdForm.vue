...
<div class="reft_screen_form p-3 " v-show="seen">
  <div class="">
          ...
          <!-- <div>
            <span>Максимальный / Итоговый вес: {{max_count}} / {{total_count}} кг.</span>
          </div> -->
          <div class="row">
            <div class="col-md-12 col-sm-12 col-xs-12">
              <div class="form-group form-group-lg">
                <BootstrapSwitcher
                label="Палетная"
                v-model:value="itPalet"
                @update="(x)=>{}" />
                <!-- <div class="div-switch">
                  <span class="span-switch">Палетная</span>
                  <label class="switch">
                    <input type="checkbox" v-model="itPalet">
                ...
                <div class="mb-3">
                  <input type="text" class="form-control bc_input" placeholder="Введите штрихкод" v-model="barcode"
                    @keyup.enter="onEnter" id="getting_prod_form_bc">
                </div>
        ...
        <div class="col-md-12 col-sm-12 col-xs-12 space">
          ...
          <GettingProdFormItem v-for="item in items" :key="item.ID" :data="item" @delete="itemDelete" />
        </div>
        <div class="navbar-fixed-bottom">
        <div class="col-md-12 col-sm-12 col-xs-12 ">
          <button class="btn btn-info btn-lg btn-block text-uppercase w-100 mb-3" @click="addManualScanning">+</button>
          <div class="btn-group w-100" role="group" >
            <button type="button" class="btn btn-warning btn-lg text-uppercase fs-6" @click="closeWithConfirm"><b>ЗАКРЫТЬ<br>ДОКУМЕНТ</b></button>
            <button type="button" class="btn btn-primary btn-lg text-uppercase fs-6" @click="clearCurrentScanings"><b>ОЧИСТИТЬ</b></button>
            <button type="button" class="btn btn-success btn-lg text-uppercase fs-6" @click="goCheck()"><b>ПРОВЕРИТЬ</b></button>
          </div>
...
const boxInOrder = computed(()=>GetCount(GettingManager.instance.currentScanings.value, 'Грузоместа'))
...
async function onScan(barcode:string){
    ...
    if (barcodeValue === "") {
        ...
        const Структура = { Штрихкод: Штрихкод, Количество: Количество, ДатаПроизводства: ДатаПроизводства, ГоденДо: ГоденДо }
  
        if (UserManager.instance.useLocalDb) {
          const barcodeFromDB = await DBManager.getFileAsync(Структура.Штрихкод, 'barcodes', 'barcodes')
          if(barcodeFromDB){
            barcodeFromDB.data.Штрихкод = Структура.Штрихкод
            barcodeFromDB.data.bc = barcodeValue
            checkScaningGetting(barcodeFromDB.data, Структура.Количество, Структура.ДатаПроизводства, Структура.ГоденДо)
          }else{
...
function checkScaningGetting(СтруктураШК:any, Количество:number,  ДатаПроизводства:string,  ГоденДо:string){
    ...
    const inOrder = prodDoc.Товары.filter(item =>
      СтруктураШК.Ссылка.Номенклатура.Наименование == item.Номенклатура.Наименование &&
      СтруктураШК.Ссылка.Характеристика.Наименование == item.Характеристика.Наименование).length > 0?true:false
  
    const ЧтоЕсть = prodDoc.Товары.filter(item => СтруктураШК.Ссылка.Номенклатура.Наименование == item.Номенклатура.Наименование)
    ...
    const КоличествоВЕдиницахИзмерения = GetCountFromBarcode(СтруктураШК, Грузоместа, Количество)
...
async function clearCurrentScanings() {
  const result = await NotificationManager.showConfirm("Вы действительно хотите удалить все текущие сканирования?")
  if(result){
